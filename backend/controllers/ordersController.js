...
export const getMyOrders = async (req, res) => {
  ...
  try {
    ...
    if (!orders.length) return res.status(404).send("No orders found for the specified product.");
...
export const createNewOrder = async (req, res) => {
  ...
  export const updateDelivered = async (req, res) => {
    ...
    try {
      // Validate the input
      if (typeof delivered !== "boolean") return res.status(400).send("Invalid delivered status.");
  
      // Find and update the order
      const order = await Order.findById(orderId);
  
      if (!order) return res.status(404).send("Order not found or you do not have access to it.");
